...
GCM=ECMWF-ERA5
EXP=evaluation
NAMETAG=EUR11_EUR11_ALADIN43_v1_ERA5_r1i1p1f1_eval

SIMULATION=${OVERRIDE_SIMULATION:-${SIMULATION}}
GCM=${OVERRIDE_GCM:-${GCM}}    # driving GCM
EXP=${OVERRIDE_EXP:-${EXP}}     # driving experiment name
NAMETAG=${OVERRIDE_NAMETAG:-${NAMETAG}} # file nameing of the input files, i.e. var_NAMETAG_date.nc

#-------------------------------------------
# Directory path settings

export BASEDIR=${HOME}/dev/repos/HCLIM2CMOR    # directory where the scripts are placed 
export DATADIR=${HCLIM_DIR}/postprocess/HCLIM2CMOR/data       # directory where all the data will be placed (typically at /scratch/)
...
INDIR_BASE=${HCLIM_DIR}/${SIMULATION}/run/archive           # base where the input is located (in sub-directories YYYY/MM/01/00)
FXDIR=${HCLIM_DIR}/${SIMULATION}/run/archive/1950/08/01/00/ # location of constant files (orog etc.)
