...
- install required packages: ``mamba install cdo nco netcdf4 pyyaml "numpy<2"``
- install package needed to update cmor table: ``mamba install pooch``
...
- remember to set the ``$HCLIMDIR`` and ``$HCLIM2CMORDIR`` env variables (see the ``config/simulation_config.yml`` file)
...
- IMPORTANT: run ``python devutil/vartable_update.py`` from the ``src`` folder regularly (daily), to make sure you have the latest info in the cmor table.
